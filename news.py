...
def get_most_popular_three_article():
    query = "select articles.title, result.num from (select " \
            "replace(path,'/article/', '') as path_slug , count(*) " \
            "as num from log group by path having path like '/article/%')" \
            "as result,articles where result.path_slug = articles.slug " \
            "order by result.num desc limit 3;"
...
def get_error_rate_of_day():
    query = "select error.dateofday::timestamp::date as tata," \
            " (sum(error.total_error)/sum( total.total_request))as " \
            "error_rate from (select time::timestamp::date  as dateofday, " \
            "count(*) as total_error from log " \
            "where log.status like '5%' or log.status like '4%' " \
            "group by dateofday) as error," \
            "(select time::timestamp::date as dateofday, " \
            "count(*) as total_request from log group by dateofday)as total" \
